...
### 이터레이션 — 코드 리뷰 후속 라운드 (보안 / a11y / 테스트 커버리지)

`code-reviewer` 에이전트 리뷰 결과 10건 모두 반영. **템플릿이 보일러플레이트를 *안전한 default*로 제시**하는 게 목적 — 다운스트림 앱이 그대로 복붙해도 보안/a11y/테스트 패턴이 깨지지 않도록:

| 영역          | 항목                                  | 결정                                                                                                                            | 이유                                                                                                                                                                    |
| ------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 보안          | `vite.config.ts` `sourcemap`          | `true` → `'hidden'`                                                                                                             | `.map` 파일은 생성하되 번들에 `sourceMappingURL` 주석 미주입 → 브라우저에서 원본 TS 노출 X. Sentry/Datadog 같은 crash-report는 `.map` 직접 업로드해 사용 — 두 마리 토끼 |
| 보안          | `RootErrorBoundary` 메시지            | `error.message` 그대로 노출 → 일반 fallback `'요청을 처리할 수 없습니다.'`                                                      | `ApiError`가 서버 에러 본문 포함 가능 — leak 방지. 상세는 `logger.error`로만 전송 (templating 시 다운스트림이 무심코 raw 노출하지 않게)                                 |
| 안정성        | `RootErrorBoundary` retry             | 없음 → `다시 시도` 버튼 (`queryClient.resetQueries()` + `navigate(0)`)                                                          | `useSuspenseQuery` + `retry: 1` 환경에서 한 번 실패하면 영구 에러 상태 — 사용자 회복 경로 필요                                                                          |
| 안전성        | MSW POST body 처리                    | `(await request.json()) as { ... }` cast → `orderInputSchema.safeParse(...)`                                                    | `request.json()` 반환은 `unknown`. 검증 없는 cast는 `undefined` 필드를 통과시켜 디버깅 어려운 silent bug 유발. 데모 코드도 안전한 패턴이어야 — 다운스트림이 따라함      |
| 테스트        | `list.test.tsx` 격리                  | `beforeEach(resetOrders)`만 → `beforeEach` + `afterEach` 둘 다                                                                  | MSW handler reset은 `vitest.setup.ts`가 처리하지만 모듈 레벨 `orders` 배열은 안 함 → POST 테스트 추가 시 다음 파일로 leak 가능. `afterEach`로 사후 격리도 보장          |
| 테스트        | `OrderForm` / `OrderNewPage` 커버리지 | 0건 → 3건 (`new.test.tsx`)                                                                                                      | mutation 흐름(submit → invalidate → navigate)이 데모의 핵심 한 축인데 미검증. 성공/zod 검증 에러/5xx mutation 에러 3 시나리오. `userEvent` setup을 `test-utils`에 추가  |
| 디자인 시스템 | `--color-success` 토큰                | `HealthStatus`가 `#22c55e` 하드코딩 (`.fail`은 `var(--color-danger)` 사용 중) → `@repo/ui` 토큰 추가                            | 일관성 + ui 패키지의 design token 완성도. local 변수로 우회하지 않고 *템플릿의 design system 자체를 보강*하는 방향 선택                                                 |
| a11y          | `HealthStatus` 색만으로 ok/fail 구분  | dot 색 (`green`/`red`) + `aria-hidden` → 보조 텍스트 (`(정상)` / `(실패)`)를 같은 라인에 시각적으로 노출                        | WCAG 1.4.1 (Use of Color) — 색만으로 정보 전달 X. `role=status + aria-live`는 동작 시점만 announce하므로 정적 텍스트가 추가로 필요                                      |
| 안정성        | `OrderList` 날짜 포맷                 | `dateFormatter.format(new Date(iso))` 직접 호출 → `formatDate(iso)` 가드 함수                                                   | `new Date('invalid')` → `Invalid Date` → `Intl.DateTimeFormat.format()`이 throw → 컴포넌트 전체 무너짐. `Number.isNaN(date.getTime())` 가드로 `—` 표시                  |
| 일관성        | `order-form` import path              | `from '../../api'` → `from '@/features/order/api'`                                                                              | CLAUDE.md "외부에서: outer barrel" 규약과 모순 (sibling 카테고리는 외부로 간주). 두 경로가 공존하면 신규 기여자가 혼란                                                  |
| 로깅          | `logger.ts` debug/info 구현           | 둘 다 `console.warn`으로 emit → `console.debug` / `console.info` per level                                                      | `console.warn`은 DevTools 경고 카운트 오염 + 다운스트림이 lint 룰로 warn 레벨을 추적할 때 false positive. `/* eslint-disable no-console */`로 logger 단독 허용          |
| 빌드          | `mockServiceWorker.js` ESLint         | husky pre-commit이 vendored worker 파일을 lint하려 함 → 루트 `eslint.config.js` ignores에 `**/public/mockServiceWorker.js` 추가 | MSW가 생성한 정적 자산 — TS project에 포함 안 되고 lint 의미도 없음                                                                                                     |

#### 회고 (이 라운드 이후 적용 가능한 일반 원칙)

- **템플릿 = "안전한 default" 책임** — 보일러플레이트 코드는 그대로 복붙되므로 *암묵적 안 좋은 패턴*도 같이 전파됨. 보안/a11y/테스트는 _보일 듯 안 보이게_ default여야 함
- **에러 메시지 두 갈래** — 사용자에게는 일반 + status code, 개발자/모니터링에는 logger로 전체 → 모든 ErrorBoundary 신규 작성 시 적용
- **MSW handler도 production-grade로** — 데모용이라도 zod 검증, 명시적 에러 응답 — 다운스트림이 그대로 패턴 차용
- **테스트 격리는 이중 안전망** — `beforeEach` 단독은 부족, `afterEach`도 함께 (다음 테스트가 무엇을 가정할지 모름)
- **디자인 토큰 부족 = 디자인 시스템의 의무** — 컴포넌트에서 임시 변수/하드코딩 발견 시 토큰을 추가하는 방향 (역방향 X)

### Phase 6에서 회수할 항목 (의도적 미적용)

react-vite에 _지금 단계에서_ 의도적으로 안 넣은 것들 — Phase 6 통합 검증 시 결정:

- **AppShell** (사이드바/탑바 레이아웃) — 페이지 4개 규모엔 과함. URL 구조가 더 복잡해지면 추가
- **ProtectedRoute** + auth feature — `stores/auth.ts`는 정의만 있고 실 사용 없음. 인증 흐름이 필요해지면
- **다크모드 토글** — `data-theme` 인프라 + `@repo/ui/theme.css` 있으나 토글 UI 없음
- **가상화 리스트** — 큰 리스트 demo 부재. `useInfiniteQuery` + `@tanstack/react-virtual`
- **차트 데모** — `/dashboard` 같은 페이지 + recharts/visx
- **`react-router` `loader` 사용 패턴** — 진입 시 prefetch 필요한 페이지가 생기면 `queryClient.prefetchQuery`
- **logger 외부 transport** (Sentry/Datadog) — 현재는 console-only
- **MSW worker prod 빌드 제외** — 현재 dynamic import + env 분기로 미로드되지만 chunk는 dist에 존재 (~267KB). 빌드 타임 분기로 chunk 자체 제거하려면 vite plugin 또는 conditional `define`
- **codegen된 실 서비스** (`features/<n>/api`가 `services/example.ts`만 사용 중) — 실제 백엔드 swagger url 확보 시
